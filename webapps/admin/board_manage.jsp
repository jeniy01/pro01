...
%>
							<tr>
								...
								<td>
									<a href="<%=path %>/admin/boardDetail.jsp?bid=<%=rs.getString("bid")%>"><%=rs.getString("title") %></a>
								</td>
								<td><%=rs.getString("resdate") %>
									<% if(!kid.equals("admin")) { %>
									<a href="<%=path %>/admin/board_del.jsp?bid=<%=rs.getString("bid")%>" class="btn btn-primary">글 삭제</a>
									<% } %>
								</td>
							</tr>
<%							
